    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < casos; i++) {
            ...
            while (!esSmith) {
                ...
                while (true) {
                    if (new BigInteger(Integer.toString(numeroCercano)).isProbablePrime(3)) {
                ...
                for (Integer factor : listaFactores) {
                    auxiliar = factor.toString();
                    for (int indiceAux = 0; indiceAux < auxiliar.toString().length(); indiceAux++) {
                        sumaPrimos = sumaPrimos + Integer.parseInt(auxiliar.substring(indiceAux, indiceAux+1));
                    }
                }

                BigInteger numeroComprobacion = new BigInteger(Integer.toString(numeroSmith));
                if (sumaDigitos == sumaPrimos) {
                    if (!numeroComprobacion.isProbablePrime(3)) {
                        esSmith = true;
                        break;
                    }
                }
